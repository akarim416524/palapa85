...
<head>
  ...
  <title>Palapa 85 - Reset Password</title>
          ...
          <div class="col-md-4 col-10 box-shadow-2 p-0">
            <div class="card border-grey border-lighten-3 m-0">
              ...
              <div class="card-content">
                <div class="card-body">
                  <form method="post" action="{{ route('reset') }}" class="form-horizontal form-simple">
                    @csrf
                    <input type="hidden" name="token" value="{{ $token }}">
                    <fieldset class="form-group position-relative has-icon-left">
                      <input type="text" class="form-control form-control-lg input-lg" value="{{ $email or old('email') }}" id="email" name="email" placeholder="Enter your email" required>
                      <div class="form-control-position">
                        <i class="ft-mail"></i>
                      </div>
                    </fieldset>
                    <fieldset class="form-group position-relative has-icon-left mb-0">
                      <input minlength="6" name="password" type="password" class="form-control form-control-lg input-lg {{ $errors->has('password') ? ' has-error' : '' }}" id="password"
                    ...
                    <fieldset class="form-group position-relative has-icon-left">
                      <input minlength="6" name="password_confirmation"  type="password" class="form-control form-control-lg input-lg {{ $errors->has('password_confirmation') ? ' has-error' : '' }}" id="password-confirm"
                    ...
                    <span class="help-block">
                      <strong><div style="color:#dc1f1f" class="error_message">{{ $errors->first('password') }}</div></strong>
                    </span>
                    @endif
                    @if ($errors->has('password_confirmation'))
          						<span class="help-block">
          							<strong><div style="color:#dc1f1f" class="error_message">{{ $errors->first('password_confirmation') }}</div></strong>
          						</span>
          					@endif

                    <button type="submit" value="Login" class="btn btn-info btn-lg btn-block"><i class="ft-unlock"></i> Submit and Login</button>
                  </form>
                </div>
              </div>
